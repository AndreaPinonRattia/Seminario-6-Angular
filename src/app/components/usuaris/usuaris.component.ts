...
import { User } from '../../models/user.model'; // Importar el modelo User desde la subcarpeta models
...
export class UsuarisComponent implements OnInit {
  ...
  desplegarBiografia: boolean[] = []; // Controla si se muestra la biografía completa
  ...
  agregarElemento(userForm: NgForm): void {
    ...
    if (this.indiceEdicion !== null) {
      ...
      const usuarioJSON: User = {
        ...
        experiences: [] // Inicializar experiences como un arreglo vacío
      };
  ...
  agregarExperiencia(usuarioId: string, event: Event): void {
    const selectElement = event.target as HTMLSelectElement; // Afirmar el tipo aquí
    const selectedValue = selectElement.value; // Obtener el valor seleccionado
    // Aquí puedes continuar con tu lógica para agregar la experiencia
  ...
  // Método para alternar entre mostrar más o menos texto en la biografía
